...
def impossible_violation_debug(unset, state, c, b_j):
    mstate = list(state)
    for i in unset:
        if c[i] > 0: mstate[i] = 1
        else: mstate[i] = 0
    best = tuple(mstate)
    dstr = "%.3lf >= %.3lf, %s"%(dot(c, mstate), b_j, (dot(c, mstate) >= b_j))
    outstr = "%s \\n => possible: %s: \\n\n"%(dstr, str(best))
    return outstr

def impossible_violation_d(unset, state, C, b):
    s = "Impossible check:\\n\n"
    for i in range(len(b)):
        ostr = impossible_violation_debug(unset, state, C[i], b[i])
        s += "(%d) %s\\n\n"%((i+1), ostr)
    return s
...
def branch(unset, state, zc, C, b):
    ...
    #tree[parent]["ip"] = "unset"
    tree[parent]["ip"] = impossible_violation_d(unset, state, C, b)
...
# Following part to get my matrix adjusted
...
def gen_dot(tree):
    ...
    for u in tree:
        if "state" in tree[u]:
            ...
            if "z" in tree[u]:
                z_str = tree[u]["z"]
            dot_str = "%d [label=\"%s\\n%s\\n%s\\n%s\n%s\"]\n"%(u, node_str, status_str, z_str, c_str, lstr)
            s += dot_str
    s += "}"
    return s
